...
class GameScene: SKScene, @preconcurrency SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        scene?.anchorPoint = .zero    // origin is at bottom left
        ...
        let moveDistance = size.width + 500
        let moveEnemies = SKAction.moveBy(x: -moveDistance, y: 0, duration: 2.75 - 0.00875 * Double(score))
        let removeEnemies = SKAction.removeFromParent()
        enemyAction = SKAction.sequence([moveEnemies, removeEnemies])
        
        let spawnAction = SKAction.run { [weak self] in
            self?.spawnEnemyObjects()
        }
        let spawnDelay = SKAction.wait(forDuration: 1.25)
    ...
    private func spawnEnemyObjects() {
        ...
        let coinFlip1 = Int.random(in: 0...1)    // offset determination
        let coinFlip2 = Int.random(in: 0...1)    // offset determination
        let offset1 = CGFloat.random(in: 0..<size.height/4) - size.height/8   // how much the spawned enemy objects will move in y-axis
        let offset2 = CGFloat.random(in: 0..<size.height/4) - size.height/8   // how much the spawned enemy objects will move in y-axis
        
        let passNode = spawnDetector()  // get the detectorNode
        let enemy1 = spawnEnemies(Int.random(in: 0..<max(1, Int(score) / 5)))   // bomb or wall?
        let enemy2 = spawnEnemies(Int.random(in: 0..<max(1, Int(score) / 5)))   // bomb or wall?
        ...
        if enemy1.size != badWallSize && enemy2.size != badWallSize { // maybe we need the 3rd obj
            if Int.random(in: 0..<max(1, Int(score))) > 8 { // lets make the 3rd obj random based on score.
                if abs(yPosition1 - yPosition2) > (4 * bombSize.height) { // if the difference in position is greater than k bomb objects, add the 3rd
                    ...
                    let offset = CGFloat.random(in: 0..<35)
                    let coinFlip = Int.random(in: 0...1)    // offset determination
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if isPlaying {
            ...
            if let loadScene = SKScene(fileNamed: "LoadScene") {
